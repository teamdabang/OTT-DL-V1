...
def youtube_link(url, message, ci, is_series=False, att=0, is_multi=False, has_drm=False, rid_map=None, user_id=0, spjc=False):
    ...
    def fetch_zee5_data(ctnid, showid=None):
        if showid:
            return requests.get(f"https://zee5-olive.vercel.app/zee5?id={ctnid}&type=EPISODE&show={showid}").json()
        return requests.get(f"https://zee5-olive.vercel.app/zee5?id={ctnid}&type=MOVIE").json()

    def extract_kid(text):
        pattern = rb'cenc:default_KID="(.*?)"/>'
        matches = re.findall(pattern, text)
        return min(matches, key=len).strip().decode() if matches else None
    ...
    def get_keys(pssh, nl, customdata):
        if pssh in pssh_cache:
            return getkid(pssh_cache[pssh])
        else:
    ...
    def check_drm_hs(datahs):
        # Implement the logic to check DRM for Hotstar
    ...
    is_dngplay = "dangalplay" in url
    is_zee5 = "zee5" in url
    is_hs = "hotstar" in url
    is_jc = "jiocinema" in url
    is_sliv = "sonyliv" in url
    is_dplus = "discoveryplus" in url

    # Handle Zee5
    if is_zee5:
        ctnid = url.split('/')[-1]
        showid = url.split('/')[-3] if "movies" not in url else None
        datazee5 = fetch_zee5_data(ctnid, showid)
        nl = datazee5['nl']
        ...
        kid = extract_kid(r.content)
        pssh = extract_pssh(r.content)

        if pssh:
            has_drm = True
            kid = get_keys(pssh, nl, customdata)

    # Handle Hotstar
    elif is_hs:
        type = "movies" if url.split('/')[-3] == "movies" else "shows"
        datahs = requests.get(url=f"https://hls-proxifier-sage.vercel.app/hotstar?type={type}", headers={'url': url, 'api': 'ottapi'}).json()
        ...
        if check_drm_hs(datahs):
            ...
            if pssh:
                logging.info("PSSH found for Hotstar")
                kid = get_keys(pssh, license_url, None)
    ...
    # Handle SonyLiv
        ...
        if datasliv["isencrypted"]:
            ...
            if pssh:
                logging.info("PSSH found for SonyLiv")
                kid = get_keys(pssh, license_url, None)
    ...
    # Process series and attachments
    ...
    if (is_sliv and datasliv["isencrypted"]) or (is_hs and check_drm_hs(datahs) or (is_zee5)):
        ...
        for lang in data['formats']:
            ...
            rid_map[frmtid] = {'kid': kid, 'pssh': pssh}
    ...
    buttons = ButtonMaker()
    if is_multi:
        buttons.ibutton(f"Multi Audio", f"d_hi_{ci}_{user_id}_None")
